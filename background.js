...
function ajustaPagina() {
  ...
  const PROFISSIONAIS = ['Alexandra', 'Domingos Jr', 'Edinho', 'Jonas', 'Juliane', 'Karine (Fisi)', 'Paula Abreu', 'Paulo Rafael', 'Rayara', 'Raphaela', 'Rayana (Fono)', 'Thais de Sa'];
  ...
  function geraLinkFormulario(linha) {
    ...
    var linkText = document.createTextNode('Link formulário:' + linha.cells[6].innerHTML);
    a.appendChild(linkText);
    a.title = 'Link formulário:' + linha.cells[6].innerHTML;
    var novoModeloData = linha.cells[0].innerHTML.substr(6, 4) + '-' + linha.cells[0].innerHTML.substr(3, 2) + '-' + linha.cells[0].innerHTML.substr(0, 2) + linha.cells[0].innerHTML.substr(10, 6);

    a.href = LINK_FORM.replace('DATA_HORA', novoModeloData);
    // Identifica o procedimento
    var servico = '';
    switch (linha.cells[7].innerHTML) {
      case '5000008':
    ...
    a.href = a.href.replace('BENEFICIARIO', linha.cells[4].innerHTML);
    a.href = a.href.replace('NUM_PROCEDIMENTO', linha.cells[7].innerHTML);
    a.href = a.href.replace('NUM_SOLICITACAO', linha.cells[6].innerHTML);
    a.target = "_blank";
